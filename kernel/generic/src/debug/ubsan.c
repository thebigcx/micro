...
void __ubsan_handle_type_mismatch_v1(struct type_mism_info* info, uintptr_t ptr)
{
    ...
    if (!ptr)
        dbglnf_crit("UBSAN: %s null pointer of type %s\n", kind, info->type->name);
    else if (ptr & (alignment - 1))
        dbglnf_crit("UBSAN: %s misaligned address %x of type %s\n", kind, ptr, info->type->name);
    else
        dbglnf_crit("UBSAN: %s address %x with insufficient space for type %s\n", kind, ptr, info->type->name);
...
{
    if (base == 0 && result == 0)
        dbgln("UBSAN: applied zero offset to nullptr\n");
    else if (base == 0 && result != 0)
        dbglnf_crit("UBSAN: applied non-zero offset %x to nullptr\n", result);
    else if (base != 0 && result == 0)
        dbglnf_crit("UBSAN: applying non-zero offset to non-null pointer %x produced null pointer", base);
    else
        dbglnf_crit("UBSAN: addition of unsigned offset to %x overflowed to %x\n", base, result);
...
{
    dbgln_crit("Stack smashing detected");
