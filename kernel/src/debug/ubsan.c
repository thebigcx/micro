...
void __ubsan_handle_type_mismatch_v1(struct type_mism_info* info, uintptr_t ptr)
{
    ...
    if (!ptr)
        dbglnf("UBSAN: %s null pointer of type %s\n", kind, info->type->name);
    else if (ptr & (alignment - 1))
        dbglnf("UBSAN: %s misaligned address %x of type %s\n", kind, ptr, info->type->name);
    else
        dbglnf("UBSAN: %s address %x with insufficient space for type %s\n", kind, ptr, info->type->name);
...
void __ubsan_handle_pointer_overflow(struct ptrover_info* info, uintptr_t base, uintptr_t result)
{
    if (base == 0 && result == 0)
        ...
        dbglnf("UBSAN: applied non-zero offset %x to nullptr\n", result);
    else if (base != 0 && result == 0)
        dbglnf("UBSAN: applying non-zero offset to non-null pointer %x produced null pointer", base);
    else
        dbglnf("UBSAN: addition of unsigned offset to %x overflowed to %x\n", base, result);
